...
def filtrer_personnel(recherche, filtre_service, groupe_nuit_actif=None, inclure_tous=False):
    """Filtre le personnel en incluant tous les employés si demandé"""
    conn = get_connection()
    if conn is None:
        return {}
    ...
    try:
        ...
        if inclure_tous:
            # Afficher TOUS les employés sans filtre de groupe
            query = """
            ...
            # Logique originale avec filtrage des groupes
            query = """
                SELECT id, nom, prenom, service, poste, heure_entree_prevue, heure_sortie_prevue, 
                       groupe_nuit, jours_travail, actif 
                FROM personnels 
        ...
        for _, row in df.iterrows():
            ...
            if recherche and recherche.lower() not in nom_complet and recherche.lower() not in service.lower() and recherche.lower() not in row['poste'].lower():
                continue
...
def get_pointage_employe_jour(personnel_id, date_pointage):
    ...
    try:
        ...
        print(f"DEBUG: Pointage trouvé pour employé {personnel_id}: {not df.empty}")
        if not df.empty:
...
def get_personnel_non_pointe():
    """Récupère le personnel qui n'a pas pointé aujourd'hui, en excluant les groupes de nuit non actifs"""
    conn = get_connection()
    if conn is None:
        return pd.DataFrame()
    ...
    try:
        ...
        return pd.read_sql_query(
            query,
            conn,
            params=(date.today(), date.today(), date.today())
        )
    except Exception as e:
...
def enregistrer_pointage_arrivee(personnel_id, date_pointage, heure_arrivee, motif_retard=None, notes=None, est_absent=False):
    ...
    try:
        ...
        with conn:
            ...
            # DEBUG: Vérifier ce qui a été enregistré
            cur.execute("SELECT heure_arrivee, statut_arrivee FROM pointages WHERE personnel_id = ? AND date_pointage = ?", 
                       (personnel_id, date_pointage))
            result = cur.fetchone()
            if result:
                print(f"DEBUG - Heure enregistrée: {result['heure_arrivee']}, Statut: {result['statut_arrivee']}")
...
def get_absences_du_jour():
    """Récupère les absences du jour en excluant les groupes de nuit non actifs"""
    conn = get_connection()
    if conn is None:
        return pd.DataFrame()
...
def approuver_conge(conge_id):
    """Approuve une demande de congé et met à jour le quota"""
    conn = get_connection()
    if conn is None:
        return False
...
def approuver_conge(conge_id):
    ...
    try:
        with conn:
            ...
            personnel_id, date_debut, date_fin = conge['personnel_id'], conge['date_debut'], conge['date_fin']
            jours_demandes = calculer_jours_conges(date_debut, date_fin)
...
def modifier_historique_pointage(pointage_id, nouvelle_heure_arrivee=None, nouvelle_heure_depart=None, nouveau_statut=None, nouveau_motif=None):
    """Modifie manuellement un pointage existant dans l'historique - VERSION CORRIGÉE"""
    conn = get_connection()
    if conn is None:
    ...
    try:
        with conn:
            ...
            # Récupérer les informations actuelles du pointage
            cur.execute(
            ...
            if nouvelle_heure_arrivee is not None:
                heure_arrivee_time = _as_time(nouvelle_heure_arrivee)
                heure_prevue = _as_time(employe_info['heure_entree_prevue'])
                
                # Recalculer le statut et le retard selon les nouvelles règles
                statut_arrivee, retard_minutes, est_absent = calculer_statut_arrivee(heure_arrivee_time, heure_prevue)
                ...
                params.extend([
                    heure_arrivee_time.strftime('%H:%M:%S'),
                    statut_arrivee,
                    retard_minutes
                ])
                ...
                # Mettre à jour la table retards si nécessaire
                cur.execute(
                    "DELETE FROM retards WHERE personnel_id = ? AND date_retard = ?",
                    (pointage_actuel['personnel_id'], pointage_actuel['date_pointage'])
                )
                
                if retard_minutes > 0 and retard_minutes < 30:
                    cur.execute(
                        "INSERT INTO retards (personnel_id, date_retard, retard_minutes, motif) VALUES (?, ?, ?, ?)",
            ...
            if nouvelle_heure_depart is not None:
                heure_depart_time = _as_time(nouvelle_heure_depart)
                ...
                params.extend([
                    heure_depart_time.strftime('%H:%M:%S'),
                    statut_depart,
                    depart_avance_minutes
                ])
            ...
            if nouveau_statut:
                ...
                # Si le statut est "Absent", supprimer les heures
                if nouveau_statut == "Absent":
                    updates.extend(["heure_arrivee = NULL", "heure_depart = NULL", "retard_minutes = 0", "depart_avance_minutes = 0"])
                    
                    # Supprimer les retards associés
                    cur.execute(
                        "DELETE FROM retards WHERE personnel_id = ? AND date_retard = ?",
                        (pointage_actuel['personnel_id'], pointage_actuel['date_pointage'])
                    )
            ...
            if updates:
                ...
                # Gérer les absences si le statut devient "Absent"
                cur.execute(
                    "DELETE FROM absences WHERE personnel_id = ? AND date_absence = ?",
                    (pointage_actuel['personnel_id'], pointage_actuel['date_pointage'])
                )
                
                if nouveau_statut == "Absent":
                    cur.execute(
                        "INSERT INTO absences (personnel_id, date_absence, motif, justifie) VALUES (?, ?, ?, ?)",
                        (pointage_actuel['personnel_id'], pointage_actuel['date_pointage'], "Absence modifiée manuellement", False)
                    )
...
def supprimer_definitivement_personnel(personnel_id):
    ...
    try:
        with conn:
            ...
            tables = [
                'retards', 'absences', 'pointages', 'conges', 'quotas_conges', 'personnels'
            ]
            ...
            for table in tables:
                try:
                    ...
                    print(f"DEBUG: Suppression dans {table} pour personnel_id {personnel_id}")
                except Exception as e:
...
def show_dashboard():
    ...
    non_pointes = get_personnel_non_pointe()  # Nouveau
    ...
    with col4:
        st.metric("Non Pointés", len(non_pointes))  # Nouveau
    
    # NOUVEAU: Personnel non pointé
...
def show_pointage_du_jour():
    ...
    with col_filtre3:
        # Option pour afficher tous les personnels (disponible pour tous les utilisateurs)
        afficher_tous = st.checkbox("Afficher tous", help="Afficher tous les employés sans filtre de groupe")
    
    # Liste du personnel filtrée - TOUS les employés si l'option est cochée
    ...
    for service, employes in personnel_filtre.items():
        ...
        for i, emp in enumerate(employes):
            ...
            with st.expander(f"{emp['prenom']} {emp['nom']} - {emp['poste']}"):
                ...
                with col_btn1:
                    if st.button("✅ Pointer l'arrivée", key=f"arr_{emp_id}_{i}"):
                ...
                with col_btn2:
                    if st.button("🚪 Pointer le départ", key=f"dep_{emp_id}_{i}"):
                        heure_reelle = datetime.now().time()
                ...
                with col_btn3:
                    if st.button("❌ Marquer absent", key=f"abs_{emp_id}_{i}"):
...
def show_gestion_personnel():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Liste du Personnel", "Ajouter un Employé", "Modifier un Employé", "Supprimer un Employé"])
                ...
                with st.form("modifier_personnel"):
                    ...
                    with col2:
                        ...
                        if poste in ["Nuit", "Mixte"]:
                            groupe_nuit = st.selectbox("Groupe de nuit", ["A", "B"], index=0 if emp_data['groupe_nuit'] == "A" else 1)
                        else:
                            groupe_nuit = "A"
                    ...
                    if poste == "Mixte":
                        ...
                        jours_actuels = emp_data.get('jours_travail', '').split(',') if emp_data.get('jours_travail') else []
                    ...
                    st.subheader("📅 Gestion des Congés")
                    col_c1, col_c2 = st.columns(2)
                    with col_c1:
                        conge_debut = st.date_input("Début du congé", value=date.today())
                        type_conge = st.selectbox("Type de congé", ["Congé annuel", "Maladie", "Familial", "Exceptionnel"])
                    with col_c2:
                        conge_fin = st.date_input("Fin du congé", value=date.today() + timedelta(days=7))
                        motif_conge = st.text_area("Motif du congé")
                    
                    accord_conge = st.checkbox("✅ Accorder ce congé", value=False,
                                             help="Cochez cette case pour enregistrer le congé")
                    
                    if st.form_submit_button("💾 Enregistrer les modifications"):
                        if poste == "Mixte" and not jours_travail:
                            st.warning("⚠️ Veuillez sélectionner au moins un jour de travail de nuit pour le personnel mixte")
                        else:
                            if modifier_personnel(emp_data['id'], nom, prenom, service, poste, heure_entree, heure_sortie, groupe_nuit, actif, jours_travail):
                                st.success("✅ Employé modifié avec succès")
                                
                                if accord_conge and conge_debut and conge_fin and conge_debut <= conge_fin:
                                    # CORRECTION: Utiliser la fonction demander_conge correctement
                                    success, message = demander_conge(emp_data['id'], conge_debut, conge_fin, type_conge, motif_conge)
                                    if success:
                                        st.success(f"✅ {message}")
                                        # Approuver automatiquement le congé
                                        try:
                                            ...
                                            if conn:
                                                with conn:
                                                    cur = conn.cursor()
                                                    cur.execute(
                                                        "SELECT id FROM conges WHERE personnel_id = ? AND date_debut = ? AND date_fin = ? ORDER BY created_at DESC LIMIT 1",
                                                        (emp_data['id'], conge_debut, conge_fin)
                                                    )
    ...
    with tab4:
        st.subheader("🗑️ Gestion des employés")
        ...
        if not personnel_actif.empty:
            employe_selection = st.selectbox(
                "Sélectionner un employé à gérer",
                ...
                try:
                    ...
                    with col1:
                        if st.button("🚫 Désactiver l'employé", type="secondary", 
                                   help="L'employé sera désactivé mais conservé dans la base de données"):
                    ...
                    with col2:
                        if st.button("🗑️ Supprimer définitivement", type="primary",
                            ...
                            st.warning("Toutes les données de cet employé seront définitivement supprimées :")
                            st.warning("- Pointages")
                            st.warning("- Absences")
                            st.warning("- Congés")
                            st.warning("- Retards")
                            st.warning("- Quotas de congés")
                            
                            confirmation = st.text_input("Tapez 'SUPPRIMER' pour confirmer la suppression définitive")
                            ...
                            if st.button("✅ Confirmer la suppression définitive", 
                                ...
                                if supprimer_definitivement_personnel(personnel_id):
                                    ...
                                    st.error("❌ Erreur lors de la suppression définitive")
                
                except (IndexError, ValueError) as e:
                    st.error("❌ Erreur lors de la sélection de l'employé")
                    print(f"DEBUG: Erreur extraction ID - {e}")
        else:
            st.info("Aucun employé actif à gérer")
            
        # Section pour gérer les employés désactivés
        st.subheader("🔄 Réactiver des employés désactivés")
        
        personnel_total = get_personnel()
        personnel_desactive = personnel_total[personnel_total['actif'] == 0]
        
        if not personnel_desactive.empty:
            employe_desactive_selection = st.selectbox(
                "Sélectionner un employé désactivé à réactiver",
                personnel_desactive.apply(lambda x: f"{x['prenom']} {x['nom']} - {x['service']} (ID: {x['id']})", axis=1),
                key="reactiver_employe"
            )
            
            if employe_desactive_selection and st.button("🔄 Réactiver l'employé"):
                try:
                    personnel_id = int(employe_desactive_selection.split('(ID: ')[1].replace(')', ''))
                    ...
                    if conn:
                        with conn:
                            cur = conn.cursor()
                            cur.execute(
                                "UPDATE personnels SET actif = 1 WHERE id = ?",
                                (personnel_id,)
                            )
                        st.success("✅ Employé réactivé avec succès")
                        time.sleep(1)
                        st.rerun()
                except Exception as e:
                    st.error(f"❌ Erreur lors de la réactivation: {e}")
        else:
            st.info("Aucun employé désactivé")
...
def show_historique_pointages():
    ...
    if st.button("🔍 Charger l'historique"):
        ...
        # AJOUT: Créer un onglet supplémentaire pour les absences
        ...
        with tab2:
            st.subheader("✏️ Modification avancée des pointages")
            
            if not pointages_df.empty:
                # Créer une liste détaillée pour la sélection
                pointages_list = []
                for _, row in pointages_df.iterrows():
                    pointages_list.append(
                        f"{row['prenom']} {row['nom']} - {row['date_pointage']} - "
                        f"Arr: {row.get('heure_arrivee', 'N/A')} - "
                        f"Statut: {row.get('statut_arrivee', 'N/A')} - "
                        f"ID: {row.get('id', 'N/A')}"
                    )
                
                pointage_selection = st.selectbox("Sélectionner un pointage à modifier", pointages_list)
                
                if pointage_selection:
                    # Extraire l'ID du pointage
                    try:
                        pointage_id = int(pointage_selection.split('ID: ')[1])
                        selected_data = pointages_df[pointages_df['id'] == pointage_id].iloc[0]
                        ...
                        with col1:
                            ...
                            heure_arrivee_actuelle = _as_time(selected_data.get('heure_arrivee')) if selected_data.get('heure_arrivee') else None
                            nouvelle_heure_arrivee = st.time_input(
                                "Heure d'arrivée",
                                value=heure_arrivee_actuelle or tm(8, 0),
                                key=f"arr_{pointage_id}"
                            )
                        ...
                        with col2:
                            st.subheader("Départ")
                            heure_depart_actuelle = _as_time(selected_data.get('heure_depart')) if selected_data.get('heure_depart') else None
                            nouvelle_heure_depart = st.time_input(
                                "Heure de départ",
                                value=heure_depart_actuelle or tm(16, 0),
                                key=f"dep_{pointage_id}"
                            )
                            
                            motif_actuel = selected_data.get('motif_retard', '')
                            nouveau_motif = st.text_area(
                        ...
                        if st.button("💾 Enregistrer les modifications", key=f"save_{pointage_id}"):
                            success, message = modifier_historique_pointage(
                                pointage_id,
                                nouvelle_heure_arrivee,
                                nouvelle_heure_depart,
                                nouveau_statut,
                                nouveau_motif
                            )
                        ...
                        # Option pour supprimer complètement le pointage
                        if st.button("🗑️ Supprimer ce pointage", type="secondary", key=f"del_{pointage_id}"):
                            if st.checkbox("⚠️ Confirmer la suppression définitive", key=f"confirm_{pointage_id}"):
                                conn = get_connection()
                                if conn:
                                    try:
                                        with conn:
                        ...
                        st.error("❌ Impossible d'extraire l'ID du pointage")
            else:
        ...
        with tab3:
            ...
            with st.form("recherche_avancee"):
                col1, col2 = st.columns(2)
                with col1:
                    nom_recherche = st.text_input("Nom")
                    prenom_recherche = st.text_input("Prénom")
                    service_recherche = st.selectbox("Service", ["Tous"] + get_services_disponibles())
                with col2:
                    date_debut_recherche = st.date_input("Date début", value=date.today() - timedelta(days=30))
                    date_fin_recherche = st.date_input("Date fin", value=date.today())
                    statut_recherche = st.selectbox("Statut", ["Tous", "Présent à l'heure", "En retard", "Absent"])
                ...
                for _, absence in absences_df.iterrows():
                    with st.expander(f"{absence['prenom']} {absence['nom']} - {absence['date_absence']}"):
                        ...
                        with col2:
                            ...
                            if absence['has_certificat']:
                                if st.button("👁️ Voir le justificatif", key=f"cert_{absence['id']}"):
                                    afficher_justificatif_absence(absence['id'])
                
                # Option d'export
                st.download_button(
                    "📥 Exporter les absences en CSV",
                    absences_df.to_csv(index=False),
                    "absences.csv",
                    "text/csv"
                )
            else:
