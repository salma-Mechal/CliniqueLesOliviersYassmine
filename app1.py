...
def filtrer_personnel(recherche, filtre_service, groupe_nuit_actif=None):
    personnel_par_service = get_personnel_par_service(groupe_nuit_actif)
    result = {}
...
def supprimer_personnel(personnel_id):
    ...
    try:
        ...
        st.error(f"Erreur suppression personnel: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def get_absences_du_jour():
    """Récupère les absences du jour actuel (hors congés approuvés)"""
    conn = get_connection()
    if conn is None:
        return pd.DataFrame()
    try:
...
def supprimer_definitivement_personnel(personnel_id):
    """Supprime définitivement un employé et toutes ses données"""
    conn = get_connection()
    if conn is None:
        return False
    ...
    try:
        with conn:
            ...
            tables = [
                'pointages', 'retards', 'absences', 'conges', 
                'quotas_conges', 'personnels'
            ]
            ...
            for table in tables:
                try:
                    ...
                    st.error(f"Erreur suppression table {table}: {e}")
                    continue
...
def show_pointage_du_jour():
    ...
    # Gestion des tours de rôle pour le personnel de nuit
    services_nuit = get_services_nuit()
    groupes_actifs = {}
    
    for service in services_nuit:
        groupes_actifs[service] = get_groupe_nuit_actif_service(service)
    
    if st.session_state.user_role == "admin":
        with st.expander("🌙 Configuration des groupes de nuit actifs"):
            for service in services_nuit:
                groupe_actuel = groupes_actifs[service]
    ...
    if services_nuit:
        st.subheader("🌙 Groupes de nuit actifs aujourd'hui")
        for service, groupe in groupes_actifs.items():
            st.info(f"**{service}**: Groupe {groupe}")
    
    # Déterminer le groupe actif pour le filtrage
    # Pour les admins, afficher tous les groupes (pas de filtrage)
    # Pour les non-admins, utiliser le groupe A par défaut
    groupe_actif_global = None
    if st.session_state.user_role != "admin":
        groupe_actif_global = "A"
        st.info(f"🔍 Affichage du personnel de jour et du groupe de nuit {groupe_actif_global}")
    ...
    personnel_filtre = filtrer_personnel(recherche, filtre_service, groupe_actif_global)
                                        ...
                                        if conn:
                                            try:
                                                with conn:
                                                    ...
                                                    conge_id = cur.fetchone()
                                                    if conge_id:
                                                        approuver_conge(conge_id['id'])
            ...
            if employe_selection:
                ...
                with col1:
                    if st.button("🚫 Désactiver l'employé", type="secondary"):
                        if supprimer_personnel(emp_data['id']):
                ...
                with col2:
                    if st.button("🗑️ Supprimer définitivement", type="primary"):
                        st.warning("⚠️ Cette action est irréversible ! Toutes les données de l'employé seront perdues.")
                        if st.checkbox("Je confirme la suppression définitive"):
                            if supprimer_definitivement_personnel(emp_data['id']):
...
def show_historique_pointages():
    ...
    if st.button("🔍 Charger l'historique"):
        ...
        with tab3:
            if not absences_df.empty:
                st.dataframe(absences_df, use_container_width=True)
            else:
                st.info("Aucune absence dans la période sélectionnée")
